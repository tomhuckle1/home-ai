...
export default function MyHomeScreen() {
  ...
  if (isLoading) {
    return (<Screen edges={['top']}><View style={{ paddingVertical: theme.spacing.md }}><Text variant="largeTitle">My home</Text></View><SkeletonList count={3} /></Screen>);
  }

  if (!property) {
    return (
      <Screen edges={['top']} style={{ justifyContent: 'center' }}>
        <EmptyState icon="🏠" title="No property yet" description="Add your property to start building its record." actionLabel="Add property" onAction={() => router.push('/property/new')} />
      </Screen>
    );
  }
...
function RoomsView({ property, refetch, isRefetching }: { property: PropertyRow; refetch: () => void; isRefetching: boolean }) {
  ...
  return (
    <FlatList
        ...
        <View style={{ gap: theme.spacing.xs }}>
          ...
          <Card onPress={() => router.push({ pathname: '/property/edit', params: { id: property.id } })}>
            <ListRow
              ...
              subtitle={[property.property_type?.replace(/_/g, ' '), property.bedrooms ? `${property.bedrooms} bed` : null, property.epc_rating ? `EPC ${property.epc_rating}` : null].filter(Boolean).join(' · ') || 'Add details like bedrooms, year built, EPC'}
              showChevron
            />
          </Card>
        </View>
      ...
      ListEmptyComponent={
        <EmptyState icon="🚪" title="No rooms yet" description="Add your first room — kitchen, bathroom, garden — to start recording what's in it." actionLabel="Add a room" onAction={() => router.push({ pathname: '/room/new', params: { propertyId: property.id } })} />
      }
      renderItem={({ item }) => (
        <Card>
          <ListRow
      ...
      ListFooterComponent={
        <View style={{ gap: theme.spacing.sm, marginTop: theme.spacing.lg }}>
          ...
          {contractorCount === 0 ? (
            <Card onPress={() => router.push({ pathname: '/contractor/new', params: { propertyId: property.id } })}>
              <ListRow leading={<Ionicons name="construct-outline" size={20} color={theme.colors.textTertiary} />} title="Save your tradespeople" subtitle="Plumbers, electricians, builders" showChevron />
            </Card>
          ) : (
...
function DocumentsView({ property }: { property: PropertyRow }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={{ paddingBottom: theme.spacing.sm }}>
        <TextField placeholder="Search by supplier, brand, product…" value={search} onChangeText={setSearch} autoCapitalize="none" />
      </View>
      {isLoading ? <ActivityIndicator /> : (
        <FlatList
          ...
          renderItem={({ item }: { item: DocumentRow }) => (
            <Card>
              <ListRow
                leading={<View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: theme.colors.surfaceAlt, alignItems: 'center', justifyContent: 'center' }}><Ionicons name={docIcon(item.document_type)} size={18} color={theme.colors.textSecondary} /></View>}
                title={item.product_description || item.original_filename || item.document_type.replace(/_/g, ' ')}
                subtitle={item.supplier ?? item.document_type.replace(/_/g, ' ')}
